namespace :queries do
  ...
  #
  # String templates for generating queries
  #
  private
  QSTART = <<-EOSTART
    SELECT DISTINCT 
  ...
  QEND = <<-EOEND
       ...
       wd:Q41710 }
    FILTER(
      NOT EXISTS { ?organization wdt:P576 _:b15. }) 
        ?organization (wdt:P31/(wdt:P279*)) 
        ?instance_of; (wdt:P131+) 
      ...
      ORDER BY (?organization) LIMIT 10000
  EOEND

  # build a default query inserting the county name into the WHERE clause
